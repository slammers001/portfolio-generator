...
async function generateWebsite(answers) {
    const websiteDir = path_1.default.join(process.cwd(), `${answers.name.toLowerCase().replace(/\s+/g, '-')}-portfolio`);
    const srcDir = path_1.default.join(websiteDir, 'src');
    const publicDir = path_1.default.join(websiteDir, 'public');
    try {
        // Create directories
      ...
      <footer className="footer">
        <div className="footer-content">
          ...
          <div className="footer-bottom">
            <p>&copy; 2025 ${answers.name.replace(/'/g, "\\'")}. Crafted with ❤️ by s009900</p>
          </div>
        </div>
      </footer>
    </div>
...
`;
        ...
        await fs_extra_1.default.ensureDir(path_1.default.join(websiteDir, 'src'));
        await fs_extra_1.default.ensureDir(path_1.default.join(websiteDir, 'public'));
        // Write files
        await Promise.all([
...
root.render(
  <React.StrictMode>
    <App />
      ...
      `),
            fs_extra_1.default.writeFile(path_1.default.join(websiteDir, 'README.md'), `# ${answers.name}'s Portfolio
...
To learn React, check out the [React documentation](https://reactjs.org/).
      `)
        ...
        console.log(chalk_1.default.cyan(`  cd ${path_1.default.basename(websiteDir)}`));
        console.log(chalk_1.default.cyan('  npm start\n'));
    }
    catch (error) {
        console.error(chalk_1.default.red('Error generating website:'), error.message || error);
        process.exit(1);
    }
}
async function main() {
    try {
        ...
        if (error.isTtyError) {
            ...
            console.error(chalk_1.default.red('An error occurred:'), error.message || error);
        }
        process.exit(1);
    }
}
